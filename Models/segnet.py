...
class FaciesSegNet(nn.Module):
    ...
    def __init__(self, n_class, out_channels=(8, 10, 30, 40, 60)):
        ...
        self.down_convs = nn.ModuleList(
            [double_conv_down(out_channels[i], out_channels[i + 1]) for i in range(len(out_channels) - 1)])
        self.down_convs.insert(0, double_conv_down(1, out_channels[0]))

        # set up decoder
        self.up_convs = nn.ModuleList(
            [double_conv_up(out_channels[i], out_channels[i - 1]) for i in range(len(out_channels) - 1, 0, -1)])
